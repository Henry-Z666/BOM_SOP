...
$row={param($file,$native) (@($file,(@($job.moving_occurrences)-join ';'),$v[0],$v[1],$v[2],(@($job.visible_occurrences)-join ';'),$spec,$audit,$native) -join "`t") }
$tsv=Join-Path $out 'pixel-v3.tsv';[IO.File]::WriteAllLines($tsv,@((& $row $cal 'true'),(& $row $base 'false')),[Text.UTF8Encoding]::new($false))
Set-CreoRuntimeEnvironment -Runtime $runtime
$cp=(Join-Path $PSScriptRoot 'build')+';'+(Join-Path $runtime.CommonFiles 'text\java\pfcasync.jar')+';'+(Join-Path $runtime.CommonFiles 'text\java\otk.jar')
Push-Location $stage;try{& $runtime.JavaCommand '--enable-native-access=ALL-UNNAMED' ('-Djava.library.path='+$runtime.NativeLibrary) '-cp' $cp 'PixelArrowBaseBatchV3' $runtime.CreoCommand $assembly $tsv 2>&1|Tee-Object (Join-Path $out 'pixel-v3-jlink.log');if($LASTEXITCODE -ne 0){throw 'PixelArrowBaseBatchV3 failed'}}finally{Pop-Location}
& (Join-Path $PSScriptRoot 'compose_pixel_arrows_v3_runner.ps1') -BaseJpeg $base -CalibrationJpeg $cal -OutputJpeg (Join-Path $out ($job.job_id+'.jpg')) -ExpectedArrowCount @($job.moving_occurrences).Count
