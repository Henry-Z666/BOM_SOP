param([Parameter(Mandatory=$true)][string]$ModelsRoot, [Parameter(Mandatory=$true)][string]$JobsJson, [int]$Limit = 0, [string]$OutputFolder = 'batch_v2')
...
foreach ($job in $jobs) {
  ...
  & (Join-Path $PSScriptRoot 'run_render.ps1') -ModelsRoot $ModelsRoot -AssemblyFile (Join-Path $ModelsRoot $job.assembly_file) -OutputJpeg ($base + '_exploded.jpg') -ExplodeComponentIds $ids -Translation @($v[0], $v[1], $v[2])
