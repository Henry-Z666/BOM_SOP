...
from .paths import INPUT_TEMPLATE, OUTPUTS, ROOT
from .validation import validate_contract


def publish(contract_path: Path) -> Path:
    ...
    subprocess.run([node, str(ROOT / "scripts" / "export_sop.mjs"), str(contract_path), str(INPUT_TEMPLATE), str(output)], check=True)
    return output
