...
- 正式任务只有两个允许的相机：接收面 1/2/3 必须使用 `fixed_123`；接收面 4/5/6 必须使用 `fixed_456`。禁止生成或选择 `receiver_emphasis`、俯仰增强/减弱、相邻八分体以及任何其它第三视角。
- 两个固定视角必须用阶段预览验证“活动件和接收位置同时可见”；若其中一张把接收面退化成线，只能选择另一张，不得新造第三视角。
...
- 每张爆炸图叠加从活动组指向接收面的一条或多条安装箭头；箭头终点必须落在已知接收面/孔位。
- 所有导出先统一为 Creo 原生 1800×2400 px（9×12 英寸、200 DPI）画布，并在 Creo 内 Refit。随后只允许固定的 1200×1600 px（3:4）单次裁切：不检测几何、不改变主体比例、不缩放，JPEG 质量 100。该固定左边界会稳定排除左下会话文字。
...
- 每张安装底图必须使用原生“仅零件”临时 Layer 隔离：只将当前阶段可见的顶层 occurrence 加入该 Layer 并设为 Display。不得按颜色、名称或图像后处理猜测/删除焊缝；所有基准、注释、透明平面、线条、焊接符号及其它非零件对象必须由 Creo 的层隔离和显示配置原生屏蔽。安装箭头只在原生纯零件底图上叠加。
# 相机合同描述格式
