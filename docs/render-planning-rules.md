...
> 当前生产规则（2026-08-20）：正式任务采用 `native_zoom_to_selected/v1`，把完整 moving occurrence 写入 Creo Selection Buffer，只调用一次 `ProCmdZoomIntoOutline`，不设置绝对 PAN/ZOOM。`zoom_to_selected_level` 按“爆炸态移动件投影尺寸 / 完整态与爆炸态联合安装包络投影尺寸”连续计算，并限制在 `[0.14, 0.42]`；安装跨度越长，镜头退得越多。每任务只允许一张正式帧，失败保留原图并提问，不执行自动构图恢复。
...
- 构图参数必须写入 `fixed-frame-presentation/v1` 白名单，并声明 `framing_priority=installation_activity/v1` 与 `zoom_anchor=installation_activity_center/v1`。worker 不能接受 Qwen、GUI 或用户任意注入相机矩阵、PAN/ZOOM 和输出路径。每张 1600×1600 方图用 `raster-composition-gate/v2` 测量主体、绿色原生箭头和边界；主体偏小、活动区偏心等纯展示问题保留真实图片并标记为待人工复核，不得伪装成 CAD/箭头结构失败。图片缺失、相机/装配事实错误、箭头审计错误仍阻止机器自动通过。正式方图不得二次动态裁切或重采样。
...
- 当 Creo 箭头审计已经通过、但栅格硬门判定箭头离屏或贴边时，保留唯一正式原图并进入人工复核；不得注入 PAN/ZOOM 或生成修正帧。
- 正式图复用当前运行工作区内同一个 Creo/J-Link Worker；每个 Worker 只打开隔离模型副本，按任务关闭窗口并擦除临时状态。Python 调度器每20个正式任务主动重启 Worker，Worker 空闲5分钟后退出。Python 与 Java 只通过工作区内原子命令、结果、就绪和心跳文件通信；每条命令完成时必须先刷新心跳再发布结果。进程消失、结果超时或超过配置空闲期限时，本代 Worker 立即失效；不得把任意路径、交互控制台或 Qwen 指令直接传给 Creo。
...
- 固定 123 与固定 456 使用同一版本化构图合同。正式 PAN 固定为零，缩放仅由原生选择包围盒合同决定；相机方向不得为构图偏移而改变。
