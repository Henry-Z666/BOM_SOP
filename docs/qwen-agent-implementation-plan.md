...
- 构图合同必须包含固定相机、安装包络尺度证据和固定画框；
- 每个步骤只执行一次原生选择包围盒构图并输出一张正式图，越界时保留原图进入复核；
