...
尺寸自适应由 Creo 对联合选择包围盒的原生 fit 完成；`0.85` 只控制 fit 之后的统一相对留白。相对原 `0.75` 基准，主体预期跨度由约 `0.55` 提高到约 `0.62`。不再根据 moving/receiver 比例二次缩放，避免接收件尺寸或爆炸距离被重复计入。接收件仍直接参与原生选择和中心计算。
